...
<div class="min-h-screen bg-gray-100 dark:bg-gray-900 py-12 px-4">
    <div class="max-w-2xl mx-auto">
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route('buyer.orders.show', $orderItem->order->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 mb-4 inline-flex items-center">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden mb-6">
            <div class="px-6 py-6 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="flex items-start gap-4">
                    @if($orderItem->product->images && $orderItem->product->images->first())
                    ...
                    @endif
                    <div class="flex-1">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $orderItem->product->name }}</h3>
                        <p class="text-gray-600 dark:text-gray-400 text-sm mt-1">Order #{{ $orderItem->order->id }}</p>
        ...
        <!-- Return Form -->
        <form action="{{ route('buyer.returns.store') }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            @csrf
                ...
                <div class="space-y-3">
                    @forelse($returnReasons as $reason)
                        <label class="flex items-start p-4 border border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            <input type="radio" name="return_reason_id" value="{{ $reason->id }}" 
                                   class="mt-1 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600"
                                   {{ old('return_reason_id') == $reason->id ? 'checked' : '' }}>
                            <div class="ml-3 flex-1">
                                <span class="font-medium text-gray-900 dark:text-white">{{ $reason->name }}</span>
                                @if($reason->description)
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $reason->description }}</p>
                                @endif
                            </div>
                        </label>
                    @empty
            ...
            <div class="px-6 py-6">
                ...
                <textarea name="custom_reason" 
                          ...
                          class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none @error('custom_reason') border-red-500 @enderror"
                          {{ old('custom_reason') }}>{{ old('custom_reason') }}</textarea>
            ...
            <div class="px-6 py-6 bg-gray-100 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-700 flex gap-3">
                <a href="{{ route('buyer.orders.show', $orderItem->order->id) }}" 
